...
- Hỗ trợ 6 vai trò: `ADMIN | OPS_MANAGER | DISPATCHER | SALES | DRIVER | CUSTOMER`
...
- Middleware `src/middleware.ts` dùng cookie `dispatch_role` để redirect DRIVER → `/driver`, CUSTOMER → `/customer`.
