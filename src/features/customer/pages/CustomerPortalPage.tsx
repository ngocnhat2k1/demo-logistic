...
import { availableQuota, quotaInUse, quotaLabel, quotaLevelColor, quotaUsageRatio } from "@/features/orders/domain/quota";
...
const CLOSED_STATUSES: OrderStatus[] = ["DELIVERED", "RETURNED", "CANCELLED", "CANCELLED_AFTER_RETURN"];
const ISSUE_STATUSES: OrderStatus[] = ["DELIVERY_FAILED", "RETURN_PROCESSING", "RETURNING_TO_WAREHOUSE", "RETURNED"];
const FILTERS = [
...
export default function CustomerPortalPage() {
  ...
  const trackingVehicle = trackingAssignment
    ...
    : undefined;
  const trackingDriver = trackingAssignment
    ? drivers.find((d) => d.id === trackingAssignment.driverId)
    : undefined;
  const trackingProgress = trackingOrder ? estimateProgress(trackingOrder, trackingVehicle?.routeProgress) : 0;
  ...
  const visibleOrders = customerOrders.filter((o) => {
    ...
    return `${o.code} ${o.description} ${o.pickup.address} ${o.dropoff.address}`.toLowerCase().includes(query);
  });
  ...
  if (trackingOrder) {
    ...
    if (trackingVehicle) {
      markers.push({
        ...
        popup: `${trackingVehicle.plateNumber} • ${trackingDriver?.fullName ?? "Tài xế"}`,
  ...
  return (
    <div className="space-y-5">
      <section className="grid gap-3 md:grid-cols-4">
        <MetricCard icon={<Package className="h-4 w-4" />} label="Tổng đơn" value={customerOrders.length} />
        <MetricCard icon={<Clock3 className="h-4 w-4" />} label="Đang xử lý" value={activeOrders.length} />
        <MetricCard icon={<CheckCircle2 className="h-4 w-4" />} label="Đã giao" value={deliveredOrders.length} />
        <MetricCard icon={<AlertTriangle className="h-4 w-4" />} label="Sự cố" value={issueOrders.length} tone={issueOrders.length > 0 ? "warn" : "muted"} />
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.35fr_0.9fr]">
          ...
          <CardHeader className="pb-3">
            <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                {trackingOrder && (
                  <p className="mt-1 text-sm text-muted-foreground">
                    {trackingOrder.code} • {ORDER_STATUS_LABEL[trackingOrder.status]}
                  </p>
                )}
              </div>
            ...
            {trackingOrder ? (
              <>
                <div className="overflow-hidden rounded-md border">
                  <MapCanvas
                    center={trackingVehicle?.currentLocation ?? trackingOrder.pickup}
                    zoom={10}
                    markers={markers}
                    polylines={polylines}
                ...
                <div className="grid gap-3 md:grid-cols-[1fr_auto_1fr] md:items-center">
                  <AddressBlock label="Điểm lấy" address={trackingOrder.pickup.address} />
                  <ArrowRight className="hidden h-4 w-4 text-muted-foreground md:block" />
                  <AddressBlock label="Điểm giao" address={trackingOrder.dropoff.address} alignRight />
                </div>
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  <InfoTile label="Xe" value={trackingVehicle?.plateNumber ?? "Chưa phân xe"} icon={<Truck className="h-4 w-4" />} />
                  <InfoTile label="Tài xế" value={trackingDriver?.fullName ?? "Chưa có"} />
                  <InfoTile label="Hẹn giao" value={formatDate(trackingOrder.requestedDeliveryAt)} icon={<CalendarClock className="h-4 w-4" />} />
                </div>
              </>
            ) : (
              <p className="text-sm text-muted-foreground">Chưa có đơn hàng để theo dõi.</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <p className="text-sm text-muted-foreground">{quotaLabel(customer.quota)}</p>
          </CardHeader>
          <CardContent className="space-y-4">
            {customer.quota.type === "POSTPAID" ? (
              <div className="grid gap-3">
                <QuotaNumber label="Công nợ chưa thu" value={formatKg(customer.quota.outstanding ?? 0)} tone={quotaState} />
                <QuotaNumber label="Đã giao tích lũy" value={formatKg(customer.quota.used)} />
                <QuotaNumber label="Đang giữ chỗ" value={formatKg(customer.quota.reserved ?? 0)} />
              </div>
            ) : (
              <>
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold">{Math.round(quotaUsageRatio(customer.quota) * 100)}%</span>
                  </div>
                  <Progress
                    value={Math.min(100, quotaUsageRatio(customer.quota) * 100)}
                    indicatorClassName={
                      quotaState === "danger" ? "bg-destructive" : quotaState === "warn" ? "bg-warning" : "bg-primary"
                    }
                    className="h-3"
                  />
                </div>
                <div className="grid gap-3">
                  <QuotaNumber label="Khả dụng" value={formatKg(availableQuota(customer.quota))} tone={quotaState === "danger" ? "danger" : "ok"} />
                  <QuotaNumber label="Đang giữ chỗ" value={formatKg(customer.quota.reserved ?? 0)} />
                  <QuotaNumber label="Đã tiêu thụ" value={formatKg(quotaInUse(customer.quota))} />
                </div>
              </>
            )}
          </CardContent>
      ...
      <section id="orders" className="space-y-3 scroll-mt-24">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{visibleOrders.length} đơn phù hợp</p>
          </div>
          ...
          <CardContent className="p-0">
            <div className="divide-y">
              ...
              {visibleOrders.map((order) => (
                <CustomerOrderRow key={order.id} order={order} vehiclePlate={vehiclePlate(order, vehicles)} />
              ))}
            </div>
          </CardContent>
        </Card>
...
function CustomerOrderRow({ order, vehiclePlate }: { order: Order; vehiclePlate: string }) {
  return (
    <Link href={`/customer/orders/${order.id}`} className="block p-4 transition hover:bg-muted/40">
      <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs md:min-w-[260px]">
          ...
          <MiniValue label="Hẹn giao" value={formatDate(order.requestedDeliveryAt, "dd/MM")} />
        </div>
      </div>
    </Link>
  );
    ...
    <Card>
      <CardContent className="flex items-center justify-between p-4">
        <div>
          ...
          <p className={cn("mt-1 text-2xl font-semibold", tone === "warn" && "text-warning", tone === "muted" && "text-muted-foreground")}>
            {value}
          </p>
        </div>
        <span className="flex h-9 w-9 items-center justify-center rounded-md bg-muted text-muted-foreground">{icon}</span>
      </CardContent>
    </Card>
  );
}

function AddressBlock({ label, address, alignRight }: { label: string; address: string; alignRight?: boolean }) {
  return (
    <div className={cn("min-w-0 rounded-md border bg-muted/30 p-3", alignRight && "md:text-right")}>
      <p className="text-xs text-muted-foreground">{label}</p>
...
function QuotaNumber({ label, value, tone = "default" }: { label: string; value: string; tone?: "default" | "ok" | "warn" | "danger" }) {
  return (
    <div className="rounded-md border bg-muted/20 p-3">
      <p className="text-xs text-muted-foreground">{label}</p>
      <p
