...
import { mulberry32, pick, rangeInt, uid } from "@/lib/utils";
...
export function buildSeed() {
  ...
  const carriers: Carrier[] = [
    ...
    { id: "carrier_backup", code: "BACKUP", name: "Nhà xe Dự phòng", type: "BACKUP", contactPhone: "02838456789" },
  ];

  // ---- Drivers (18) ----
  const drivers: Driver[] = [];
  for (let i = 0; i < 18; i++) {
    const carrierId = pick(rng, carriers).id;
    drivers.push({
      id: `driver_${i + 1}`,
  ...
  for (let i = 0; i < 25; i++) {
    const carrierId = i < 8 ? "carrier_ttm" : i < 16 ? "carrier_vna" : i < 21 ? "carrier_dxp" : "carrier_backup";
